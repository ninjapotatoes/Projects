...
public class SeparateChainingHashMap<K extends Comparable<K>, V> extends AbstractHashMap<K, V> {
    ...
    public V bucketPut(int hash, K key, V value) {
        ...
        int oldSize = buck.size();
        
        V placeholder = buck.put(key, value);
        
        size += (buck.size() - oldSize);
        
        return placeholder;
    }

    /**
	 * In bucketRemove it is based on page 425 the ChainHashMap class on page 425 in the
	 * course textbook "Data Structures and Algorithms" by Goodrich, Tamassia,
	 * Goldwasser.
	 */
    @Override
    public V bucketRemove(int hash, K key) {
    	...
    	// if bucket is null then return null
        if (buck == null) {
            return null;
        }
        ...
        V returnVal = buck.remove(key);
        
        size -= oldSize - buck.size();
