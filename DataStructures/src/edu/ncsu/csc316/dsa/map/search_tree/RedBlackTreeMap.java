...
public class RedBlackTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void resolveRed(Position<Entry<K, V>> p) {
        ...
        if (isRed(parent)) {
            boolean parentIsLeftChild = parent == left(grandparent);

            Position<Entry<K, V>> uncle = parentIsLeftChild ? right(grandparent) : left(grandparent);

            // CASE 1: The uncle is black or null
