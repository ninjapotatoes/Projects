...
public class HeapAdaptablePriorityQueue<K extends Comparable<K>, V> extends HeapPriorityQueue<K, V>
    ...
    public void remove(Entry<K, V> entry) {
        AdaptablePQEntry<K, V> adaptableEntry = validate(entry);
        
        int index = adaptableEntry.getIndex();
        
        //If there is nothing after just remove it
        if (index == size() - 1) {
            list.remove(index);
        } else {
        	//make last entry first
            swap(index, size() - 1);
            
            //get rid of original first
            list.remove(size() - 1);
    ...
    public void replaceKey(Entry<K, V> entry, K key) {
        ...
        adaptableEntry.setKey(key);
        
        bubble(adaptableEntry.getIndex());
    }
