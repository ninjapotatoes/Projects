...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    protected void upHeap(int index) {
    	...
    	//compare
        while (index > 0 && compare(list.get(index).getKey(), list.get(parentIndex).getKey()) < 0) {
    ...
    protected void downHeap(int index) {
    	while (hasLeft(index)) {
            ...
            int leftChild = leftIndex;
            
            //if theres a right check that its smalelr
            if (hasRight(index)) {
                int rightIndex = right(index);
                if (compare(list.get(leftIndex).getKey(), list.get(rightIndex).getKey()) > 0) {
                    leftChild = rightIndex;
                }
            }
            
            //if current > smallest child swap
            if (compare(list.get(leftChild).getKey(), list.get(index).getKey()) < 0) {
